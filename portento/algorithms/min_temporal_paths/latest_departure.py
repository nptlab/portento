...
def latest_departure_time(stream: Stream, source: Hashable, time_bound: Interval = None):
    if not time_bound:
        time_bound = stream.stream_presence.root.full_interval

    start, end = get_start_end(time_bound, stream.instant_duration)

    arrival_time = dict(((u, end if u == source else float('-inf')) for u in stream.nodes))
