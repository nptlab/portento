...
def find(n, interval, u, v):
    if n:
        if n.value == interval and n.u == u and n.v == v:
            return n
        elif n.value > interval:
            return find(n.left, interval, u, v)
        else:
            return find(n.right, interval, u, v)

    raise Exception(f"Node {interval, u, v} not found")
...
class TestStreamTree:
    ...
    @pytest.mark.parametrize('new_link,result', [
        (Link(Interval(3.0, 8.0, 'both'), 'a', 'b'), [Link(Interval(1.0, 8.0, 'both'), 'a', 'b'),
                                                      Link(Interval(4.5, 7.5, 'both'), 'a', 'c'),
                                                      Link(Interval(6.0, 9.0, 'both'), 'b', 'c'),
                                                      Link(Interval(2.0, 3.0, 'both'), 'b', 'd')]),
        (Link(Interval(5.0, 6.0), 'b', 'c'), [Link(Interval(1.0, 3.0, 'both'), 'a', 'b'),
                                              Link(Interval(7.0, 8.0, 'both'), 'a', 'b'),
                                              Link(Interval(4.5, 7.5, 'both'), 'a', 'c'),
                                              Link(Interval(5.0, 9.0, 'right'), 'b', 'c'),
                                              Link(Interval(2.0, 3.0, 'both'), 'b', 'd')])
    ])
    def test_add_overlap(self, links, new_link, result):
        tree = StreamTree(links)
        tree.add(new_link)
        assert list(tree) == sorted(result)
